...
###########################
ZOOKEEPER_SERVERS_ESCAPED=" -c storm.zookeeper.servers=\"[\\\"prostor-master\\\"]\""
...
########################
HOST=" -c storm.local.hostname=prostor-slave"
# For the nimbus, apply "nimbus" as default hostname
for arg in "$@"
do
    if [[ $arg == "nimbus" ]] ; then
        HOST=" -c storm.local.hostname=\"prostor-master\""
    fi
done
...
# nimbus.seeds #
################
NIMBUS_SEEDS=" -c nimbus.seeds=\"[\\\"prostor-master\\\"]\""
